...
pub struct Block {
    ...
    pub difficulty: u64,
    pub miner: Vec<u8>,
    pub prev_mined: Option<generated::proto::Hash>,
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(26) => msg.prev_mined = Some(r.read_message::<generated::proto::Hash>(bytes)?),
...
impl MessageWrite for Block {
    fn get_size(&self) -> usize {
        ...
        + self.prev_mined.as_ref().map_or(0, |m| 1 + sizeof_len((m).get_size()))
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if let Some(ref s) = self.prev_mined { w.write_with_tag(26, |w| w.write_message(s))?; }
